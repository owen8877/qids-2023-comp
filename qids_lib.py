...
class QIDS:
    def __init__(self, path_prefix: str = './') -> None:
        self.__submission_path = path_prefix + SUBMISSION_PATH
    ...
    def get_current_market(self):
        ...
        fundamental_df = self.__fundamental_df.iloc[
                         self.__current_idx * self.__num_of_stocks: (self.__current_idx + 1) * self.__num_of_stocks]
        market_df = self.__market_df.iloc[self.__current_idx * self.__num_of_stocks * self.__point_per_day
